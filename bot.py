...
def wow_action(chat_id: int, kind: str = "load") -> int | None:
    try:
        ...
        pass
    try:
        msg = bot.send_message(chat_id, _WOW_TEXTS.get(kind, "⏳  _Зачекайте..._"), parse_mode="Markdown")
        return msg.message_id
    except Exception:
        return None

def wow_done(chat_id: int, msg_id: int | None, text: str, parse_mode: str = None, reply_markup=None):
    if msg_id:
        try:
...
def reset_to_main(message):
    ...
    if uid == ADMIN_ID:
        ...
        if partner_id:
            ...
            try:
                bot.send_message(partner_id, "👋 Адміністратор завершив чат.", reply_markup=main_menu_markup(partner_id))
            except Exception:
                pass
...
def show_our_trainers(message):
    ...
    if not db:
        ...
        return
    try:
        trainers = db.execute("SELECT name, username, description FROM trainers ORDER BY name").rows
    except Exception as e:
        wow_done(cid, loader, f"❌ Помилка: {e}")
        return
    if not trainers:
        wow_done(cid, loader, "😔 Тренерів поки немає. Перевірте пізніше!", reply_markup=main_menu_markup(message.from_user.id))
        return
    wow_done(cid, loader, f"👨\u200d🏫 *Наші тренери*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nЗнайдено фахівців: *{len(trainers)}*", parse_mode="Markdown")
    for i, row in enumerate(trainers, 1):
        name  = str(_unpack_turso_value(row[0]) or "Без імені")
        uname = str(_unpack_turso_value(row[1]) or "no_username")
        desc  = str(_unpack_turso_value(row[2]) or "Опис не вказано")
        card  = format_trainer_card(name, uname, desc, index=i)
        bot.send_message(cid, card, parse_mode="Markdown")
    bot.send_message(cid, "╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌\n🎯 Готові розпочати?\nНатисніть *♟️ Вибрати тренера* щоб записатись\\!", parse_mode="MarkdownV2", reply_markup=main_menu_markup(message.from_user.id))
...
def admin_panel(message):
    ...
    bot.send_message(message.chat.id, "⚙️ *Адмін\\-панель*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nОберіть дію:", parse_mode="MarkdownV2", reply_markup=admin_menu_markup())
...
def admin_back(message):
    ...
    # ==========================================================
# ➕ ДОДАТИ ТРЕНЕРА (через переслане повідомлення)
# ==========================================================
...
def add_trainer_start(message):
    ...
    user_states[message.chat.id] = "add_trainer_wait_forward"
    ...
    bot.send_message(
        ...
        "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n"
        "🔵 Крок 1 з 3  ●○○\n\n"
        "📩 *Перешліть будь-яке повідомлення від тренера*, якого хочете додати.\n"
        "Це може бути текст, фото, голосове тощо — головне, щоб воно було *переслане від тренера*.\n\n"
        "➡️ Як це зробити:\n"
        "1. Відкрийте чат з тренером\n"
        "2. Натисніть і утримуйте на будь-якому його повідомленні\n"
        "3. Виберіть «Переслати»\n"
        "4. Виберіть цього бота\n\n"
        "Після пересилання бот автоматично отримає Telegram ID тренера.",
        parse_mode="Markdown",
        reply_markup=markup
    )
...
@bot.message_handler(
    func=lambda m: (m.from_user.id == ADMIN_ID
                    and user_states.get(m.chat.id) == "add_trainer_wait_forward")
)
def add_trainer_forward_received(message):
    cid = message.chat.id
    if not message.forward_from:
        bot.send_message(
            cid,
            "❌ *Помилка:* це повідомлення не переслане від користувача.\n\n"
            "Будь ласка, перешліть *оригінальне повідомлення тренера* (воно має містити позначку «Forwarded from ...»).",
            parse_mode="Markdown",
    ...
    first_name = fwd.first_name or ""
    last_name = fwd.last_name or ""
    full_name = f"{first_name} {last_name}".strip() or "Без імені"

    db = get_db()
    if db:
        try:
            existing = db.execute("SELECT id FROM trainers WHERE telegram_id = ?", [tg_id]).rows
            if existing:
                bot.send_message(
                    cid,
                    f"⚠️ *Тренер вже існує в базі!*\n"
                    f"┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n"
                    f"👤 {full_name}\n🆔 `{tg_id}`\n@{username}\n\nДодавання скасовано.",
                    parse_mode="Markdown",
            ...
            logger.error(f"Помилка перевірки дубліката: {e}")
    ...
    bot.send_message(
        cid,
        f"➕ *Додавання тренера*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n🔵 Крок 2 з 3  ●●○\n\n"
        f"✅ Отримано дані тренера:\n👤 {full_name}\n🆔 `{tg_id}`\n📎 @{username if username else 'немає'}\n\n"
        f"✍️ Введіть повне ім'я тренера\n_(або надішліть_ `.` _щоб використати ім'я з контакту:_ *{full_name}*_)_",
        parse_mode="Markdown",
        reply_markup=cancel_only_markup()
    )

@bot.message_handler(func=lambda m: user_states.get(m.chat.id) == "add_trainer_name" and m.from_user.id == ADMIN_ID)
...
@bot.message_handler(func=lambda m: user_states.get(m.chat.id) == "add_trainer_description" and m.from_user.id == ADMIN_ID)
def add_trainer_description(message):
    ...
    try:
        db.execute(
            ...
            [data["telegram_id"], data.get("username"), data["name"], data["description"]]
        )
        wow_done(
            cid, loader,
            f"✅ *Тренер доданий!*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n👨‍🏫 {data['name']}\n🆔 `{data['telegram_id']}`\n📎 @{data.get('username', 'немає')}\n\nТренер з'явиться у списку одразу.",
            reply_markup=admin_menu_markup()
        )
        logger.info(f"✅ Додано тренера: {data['name']} tg_id={data['telegram_id']}")
    except Exception as e:
        if "UNIQUE" in str(e) or "unique" in str(e).lower():
            wow_done(cid, loader, f"⚠️ Тренер з Telegram ID `{data['telegram_id']}` вже існує в базі.", reply_markup=admin_menu_markup())
        else:
...
# ==========================================================
# ➖ ВИДАЛИТИ ТРЕНЕРА (з використанням telegram_id)
# ==========================================================
...
def delete_trainer_start(message):
    ...
    if not db:
        ...
        return
    try:
        trainers = db.execute("SELECT telegram_id, name, username FROM trainers ORDER BY name").rows
    except Exception as e:
        wow_done(cid, loader, f"❌ Помилка: {e}")
        return
    if not trainers:
        ...
        return
    wow_done(cid, loader, f"🗑 *Видалення тренера*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nТренерів у базі: *{len(trainers)}*\n\n⚠️ _Буде запит на підтвердження перед видаленням_", parse_mode="Markdown")
    for row in trainers:
        tg_id = _unpack_turso_value(row[0])
        name  = str(_unpack_turso_value(row[1]) or "Без імені")
        uname = str(_unpack_turso_value(row[2]) or "no_username")
        card = f"👨‍🏫 {name}\n📎 @{uname}"
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🗑 Вилучити", callback_data=f"delete_{tg_id}"))
        bot.send_message(cid, card, reply_markup=markup)
...
def delete_trainer_handler(call):
    if call.from_user.id != ADMIN_ID:
        ...
        return
    try:
        tg_id = int(call.data.split("_", 1)[1])
    ...
    if not db:
        ...
        return
    try:
        result = db.execute("SELECT name, username FROM trainers WHERE telegram_id = ?", [tg_id])
        if not result.rows:
            bot.answer_callback_query(call.id, "❌ Тренера не знайдено", show_alert=True)
            return
        name  = str(_unpack_turso_value(result.rows[0][0]))
        uname = str(_unpack_turso_value(result.rows[0][1]))
    except Exception as e:
        bot.answer_callback_query(call.id, f"❌ {str(e)[:80]}", show_alert=True)
        return
    confirm_markup = types.InlineKeyboardMarkup(row_width=2)
    confirm_markup.add(
        types.InlineKeyboardButton("✅ Так, видалити", callback_data=f"confirm_delete_{tg_id}"),
        types.InlineKeyboardButton("❌ Скасувати", callback_data=f"cancel_delete_{tg_id}")
    )
    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        f"🗑 *Видалення тренера*\n╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌\n👨\u200d🏫 *{name}*\n📎 @{uname}\n\n⚠️ Ви впевнені\\? Це незворотня дія\\.",
...
def confirm_delete_trainer(call):
    ...
    if not db:
        ...
        return
    try:
        result = db.execute("SELECT name FROM trainers WHERE telegram_id = ?", [tg_id])
        if not result.rows:
            bot.answer_callback_query(call.id, "❌ Тренера вже немає в базі", show_alert=True)
            bot.edit_message_text("ℹ️ Тренера вже було видалено раніше.", call.message.chat.id, call.message.message_id)
            return
        name = str(_unpack_turso_value(result.rows[0][0]))
        db.execute("DELETE FROM trainers WHERE telegram_id = ?", [tg_id])
        logger.info(f"🗑 Видалено тренера: {name} (tg_id={tg_id})")
        bot.answer_callback_query(call.id, f"✅ {name} видалений!")
        bot.edit_message_text(f"✅ *Готово\\!*\n╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌\nТренер *{name}* видалений з бази\\.", call.message.chat.id, call.message.message_id, parse_mode="MarkdownV2")
...
def cancel_delete_trainer(call):
    ...
    bot.edit_message_text("↩️ Видалення скасовано\\.", call.message.chat.id, call.message.message_id, parse_mode="MarkdownV2")
...
def list_trainers(message):
    ...
    if not trainers:
        ...
        return
    wow_done(cid, loader, f"📋 *Тренери в базі*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nЗнайдено: *{len(trainers)}* чол\\.", parse_mode="MarkdownV2")
    for i, row in enumerate(trainers, 1):
        name  = str(_unpack_turso_value(row[0]) or "Без імені")
        uname = str(_unpack_turso_value(row[1]) or "no_username")
        desc  = str(_unpack_turso_value(row[2]) or "Опис не вказано")
        ...
        bot.send_message(cid, card, parse_mode="Markdown")
        # ==========================================================
# 👤 ВИБІР ТРЕНЕРА (користувач) – оновлена версія з telegram_id
# ==========================================================
...
def user_got_level(message):
    ...
    if not db:
        ...
        return
    try:
        trainers = db.execute("SELECT telegram_id, name, description FROM trainers ORDER BY name").rows
    except Exception as e:
        wow_done(cid, loader, f"❌ Помилка: {e}")
        reset_to_main(message)
        return
    ...
    if not trainers:
        wow_done(cid, loader, "😔 Тренерів поки немає. Спробуйте пізніше.", reply_markup=main_menu_markup(message.from_user.id))
        user_states.pop(cid, None)
        user_form.pop(cid, None)
        return

    wow_done(cid, loader,
             f"✅ *Рівень збережено\\!*\n`{level_badge}`\n\n🔵 Крок 6 з 6  ●●●●●●\n\n👇 *Оберіть тренера*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nНатисніть кнопку під карткою тренера:",
             parse_mode="MarkdownV2",
             reply_markup=cancel_only_markup())

    for row in trainers:
        tg_id_raw = _unpack_turso_value(row[0])
        ...
        pick_markup.add(types.InlineKeyboardButton("📚 Записатися на курс", callback_data=f"pick_{tg_id}"))
...
def user_picked_trainer(call):
    ...
    try:
        trainer_tg_id = int(call.data.split("_")[1])
    except (IndexError, ValueError):
        bot.answer_callback_query(call.id, "❌ Некоректні дані", show_alert=True)
        return
    ...
    if not db:
        ...
        return
    try:
        result = db.execute("SELECT username, name, telegram_id FROM trainers WHERE telegram_id = ?", [trainer_tg_id]).rows
        if not result:
    ...
    try:
        bot.edit_message_text("⚙️  _Відправляємо заявку..._", cid, call.message.message_id, parse_mode="Markdown")
    except Exception:
        pass
    time.sleep(0.8)
    try:
        bot.edit_message_text(
            f"📨 *Заявку надіслано\\!*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n👨\u200d🏫 Тренер: *{trainer_name}*\n\n⏳ _Очікуйте підтвердження адміністратора\\._\nМи одразу повідомимо вас\\!",
            cid, call.message.message_id, parse_mode="MarkdownV2"
        )
    except Exception:
        pass
    ...
    admin_msg = (
        ...
        f"👤 *{data['name']}* \\({user_tg}\\)\n📱 `{data['phone']}`\n🧒 {age_type}  │  📚 {lesson_type}\n♟️ `{level_badge}`\n\n"
        f"👨\u200d🏫 Тренер: *{trainer_name}* \\(@{trainer_username}\\)\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nПідтвердіть або відхиліть запис:"
    )
    confirm_markup = types.InlineKeyboardMarkup()
    confirm_markup.row(
        types.InlineKeyboardButton("✅ Підтвердити", callback_data=f"confirm_enroll_{cid}_{trainer_tg_id}"),
        types.InlineKeyboardButton("❌ Відхилити",   callback_data=f"reject_enroll_{cid}_{trainer_tg_id}")
    )
    try:
...
def admin_confirm_enroll(call):
    ...
    try:
        ...
        bot.edit_message_text(call.message.text + "\n\n✅ _Підтверджено адміністратором._", call.message.chat.id, call.message.message_id, parse_mode="Markdown")
    except Exception:
        pass
    try:
        bot.send_message(user_cid,
            f"🎉 *Запис підтверджено\\!*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n👨\u200d🏫 Тренер: *{trainer_name}*\n\n⏳ Адміністратор незабаром зв'яжеться з вами для узгодження деталей занять\\.\n\n_Дякуємо за довіру\\! Бажаємо успіхів\\! ♟️_",
            parse_mode="MarkdownV2"
        )
    except Exception as e:
        logger.warning(f"Не вдалося надіслати учню {user_cid}: {e}")
    confirm_trainer_msg = (
        f"✅ *Адмін підтвердив запис\\!*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nДо вас записався новий учень:\n👤 *{user_name}*\n📱 `{user_phone}`\n🧒 {age_type}  │  📚 {lesson_type}\n♟️ `{user_level_badge}`\n\n_Зв'яжіться з учнем для організації занять\\!_"
    )
    trainer_notified = False
    if trainer_tg_id:
        try:
...
def admin_reject_enroll(call):
    ...
    trainer_form[ADMIN_ID] = {"user_cid": user_cid, "trainer_name": trainer_name, "trainer_tg_id": trainer_tg_id}
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
        bot.edit_message_text(call.message.text + "\n\n⏳ Очікуємо причину від адміністратора...", call.message.chat.id, call.message.message_id, parse_mode="Markdown")
    except Exception:
        pass
    bot.send_message(call.message.chat.id,
        "❌ *Відхилення запису*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nНапишіть причину відмови для учня:",
        parse_mode="Markdown", reply_markup=cancel_only_markup())
...
def admin_write_reject_reason(message):
    ...
    try:
        bot.send_message(user_cid,
            f"😔 *Запис відхилено*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n👨\u200d🏫 Тренер: *{trainer_name}*\n\n📝 *Причина:*\n{reason}\n\n_Спробуйте обрати іншого тренера або зверніться до адміністратора\\._",
            parse_mode="MarkdownV2", reply_markup=main_menu_markup(user_cid)
        )
    except Exception as e:
        logger.warning(f"Не вдалося надіслати учню {user_cid}: {e}")
...
def chat_accept(call):
    ...
    bot.send_message(call.from_user.id, f"💬 *Чат розпочато*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nСпілкуєтесь з користувачем `{uid}`\n\n_Натисніть_ ❌ _Скасувати, щоб завершити чат_", parse_mode="Markdown", reply_markup=admin_kb)
    user_kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    user_kb.add(BTN_CANCEL)
    bot.send_message(uid, "✅ *Адміністратор на зв'язку\\!*\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\nПишіть ваше запитання\\.\n\n_Натисніть_ ❌ _Скасувати, щоб завершити чат_", parse_mode="MarkdownV2", reply_markup=user_kb)
...
def chat_reject(call):
    ...
    bot.send_message(uid, "😔 *Адміністратор зараз недоступний*\n\n_Спробуйте пізніше або оберіть іншу дію_", parse_mode="Markdown", reply_markup=main_menu_markup(uid))

@bot.message_handler(func=lambda m: user_states.get(m.chat.id) == "in_admin_chat" and m.chat.id in admin_chats)
...
@bot.message_handler(func=lambda m: m.from_user.id == ADMIN_ID and any(a == ADMIN_ID for a in admin_chats.values()) and m.text not in ADMIN_RESERVED)
...
def debug_db():
    try:
        payload = {"requests": [{"type": "execute", "stmt": {"sql": "SELECT telegram_id, name, username, description FROM trainers"}}, {"type": "close"}]}
        raw = requests.post(f"{TURSO_URL}/v2/pipeline", json=payload, headers={"Authorization": f"Bearer {TURSO_TOKEN}", "Content-Type": "application/json"}, timeout=10)
        db = get_db()
        rows = []
        if db:
            try:
                rows = [list(r) for r in db.execute("SELECT telegram_id, name, username, description FROM trainers").rows]
            except Exception as e:
                rows = [f"ERROR: {e}"]
        return json.dumps({"http_status": raw.status_code, "raw_turso_response": raw.json(), "parsed_rows": rows}, ensure_ascii=False, indent=2), 200, {"Content-Type": "application/json"}
    except Exception as e:
